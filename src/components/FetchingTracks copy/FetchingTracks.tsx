...
export default function FetchingTracks() {
  ...
  useEffect(() => {
    // Если треки уже загружены или загрузка идёт — ничего не делаем
    if (allTracks.length > 0 || fetchIsLoading) return;
    ...
    getTracks()
      ...
      .catch((error) => {
        if (error instanceof AxiosError) {
          if (error.response) {
            dispatch(setFetchError(error.response.data?.message || 'Ошибка загрузки треков'));
  ...
  }, [dispatch, allTracks.length, fetchIsLoading]);
